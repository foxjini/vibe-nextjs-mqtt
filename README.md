...
## 📥 클론 후 최초 설치 및 실행 방법 (Quick Start)

학생들이 학교 실습실 PC에서 GitHub 레포지터리를 클론한 후 처음 설치하고 구동하는 방법입니다.

### 1단계: 사전 준비
- **Node.js 설치**: PC에 Node.js (LTS 버전, v18 이상 권장)가 설치되어 있어야 합니다.  
  *(터미널에서 `node -v` 명령어로 설치 여부 확인)*
- **웹 브라우저**: **Google Chrome** 사용을 권장합니다. (내장 `Google 한국어` 음성 엔진 최적화)

### 2단계: 프로젝트 클론 및 패키지 설치
PowerShell 또는 명령 프롬프트(CMD)를 열고 다음 명령어를 순서대로 입력합니다:

```bash
# 1. 깃허브에서 소스 코드 복제 (클론)
git clone https://github.com/foxjini/vibe-nextjs-mqtt.git
...
npm run dev
```
터미널에 `Ready in ...` 메시지가 나타나면 성공입니다.

### 4단계: 브라우저 접속 및 초기 설정
1. **Chrome 브라우저**를 열고 [http://localhost:3000](http://localhost:3000) 주소로 접속합니다.
2. 화면 상단 우측의 **`[오디오 켜기 (클릭)]`** 버튼을 누릅니다.  
   *(Chrome의 자동 재생 차단 정책을 해제하여 음성 안내가 활성화됩니다.)*
3. 화면 상단 우측의 **`[설정]`** 버튼을 클릭합니다.
4. **`원격 윈도 PC Mosquitto IP 바로 입력`**란에 선생님(또는 브로커) PC의 IP 주소를 입력하고 **`[주소 생성]`** $\rightarrow$ **`[저장 및 재연결]`**을 누릅니다.
5. 상단 통신 상태가 **`[MQTT 연결됨]`** 초록색으로 켜지면 준비 완료입니다!
